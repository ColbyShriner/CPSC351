...
function check_login($con) {
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            // If user data cannot be retrieved, redirect to login page or handle the error accordingly
            header("Location: login.php");
            exit();
        }
    } else {
        // If user ID session variable is not set, redirect to login page or handle the error accordingly
        header("Location: login.php");
        exit();
    }
}
...
// Check if $con is properly initialized
if (!$con) {
...
if (isset($_POST['add_friend'])) {
    ...
    $query = "SELECT * FROM users WHERE user_name = '$friend_username'"; // Assuming 'user_name' is the column name for the username
    $result = mysqli_query($con, $query);
    $friend = mysqli_fetch_assoc($result);

    if ($friend) {
        // Get current user's ID
        $user_id = $user_data['user_id']; // Assuming 'user_id' is the column name for the user ID

        // Check if the friendship already exists
        $friend_id = $friend['user_id']; // Assuming 'user_id' is the column name for the user ID
...
while ($row = mysqli_fetch_array($result)) {
    ...
    // Assuming 'user_id' is the column name for the user ID
echo '<a href="add_friend.php?remove_friend=' . $row['user_id'] . '" style="color: white;">Remove Friend</a>';
    echo '</div>';
}
?>
    </section>
